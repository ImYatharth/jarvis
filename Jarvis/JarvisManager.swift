...
final class JarvisManager: ObservableObject {
    ...
    let wakeWordListener = JarvisWakeWordListener()
    ...
    private var wakeWordSilenceTimer: Timer?
    private var wakeWordTurnHasHeardSpeech = false
    private var wakeWordTurnStartedAt: Date?
    private var wakeWordTurnLastVoiceAt: Date?
    ...
    @Published var isWakeWordEnabled: Bool = JarvisSettings.shared.wakeWordEnabled

    func setWakeWordEnabled(_ enabled: Bool) {
        isWakeWordEnabled = enabled
        // The settings publisher (see bindWakeWord) drives the listener so the
        // UI path and any external change converge on the same start/stop code.
    ...
    func stop() {
        ...
        wakeWordSilenceTimer?.invalidate()
        wakeWordSilenceTimer = nil
    ...
    private func bindWakeWord() {
        wakeWordListener.onWakeWordDetected = { [weak self] in
            self?.handleWakeWordTriggered()
        }

        // @Published delivers the current value on subscription, so this also
        // performs the initial start when wake word is already enabled.
        wakeWordCancellable = settings.$wakeWordEnabled
            .receive(on: DispatchQueue.main)
            .sink { [weak self] enabled in
                guard let self else { return }
                self.isWakeWordEnabled = enabled
                if enabled {
                    Task { @MainActor [weak self] in
                        guard let self else { return }
                        await self.jarvisDictationManager.requestInitialPushToTalkPermissionsIfNeeded()
                        guard self.isWakeWordEnabled else { return }
    ...
    /// Starts a hands-free dictation turn after the wake word is heard. Mirrors
    /// the `.pressed` branch of the shortcut handler, then auto-stops on silence
    /// since there's no key release to end the turn.
    private func handleWakeWordTriggered() {
        ...
        pendingKeyboardShortcutStartTask = Task { [weak self] in
            guard let self else { return }
            await self.jarvisDictationManager.startPushToTalkFromKeyboardShortcut(
                currentDraftText: "",
                updateDraftText: { _ in },
                submitDraftText: { [weak self] finalTranscript in
                    self?.lastTranscript = finalTranscript
                    print("🗣️ Jarvis received transcript (wake word): \(finalTranscript)")
                    JarvisAnalytics.trackUserMessageSent(transcript: finalTranscript)
                    self?.routeTranscript(finalTranscript)
                }
            )
            self.startWakeWordSilenceMonitor()
        }
    }

    private func startWakeWordSilenceMonitor() {
        wakeWordSilenceTimer?.invalidate()
        wakeWordTurnHasHeardSpeech = false
        wakeWordTurnStartedAt = Date()
        wakeWordTurnLastVoiceAt = nil

        let speechThreshold: CGFloat = 0.06
        let silenceWindow: TimeInterval = 1.6
        let noSpeechTimeout: TimeInterval = 6.0
        let hardCap: TimeInterval = 15.0

        let timer = Timer(timeInterval: 0.15, repeats: true) { [weak self] _ in
            Task { @MainActor [weak self] in
                guard let self else { return }
                guard self.jarvisDictationManager.isActivelyRecordingAudio
                    || self.jarvisDictationManager.isPreparingToRecord else {
                    // Dictation ended on its own — clean up and resume listening.
                    self.finishWakeWordTurn(submit: false)
                    return
                }

                let now = Date()
                let level = self.jarvisDictationManager.currentAudioPowerLevel
                if level > speechThreshold {
                    self.wakeWordTurnHasHeardSpeech = true
                    self.wakeWordTurnLastVoiceAt = now
                }

                if let started = self.wakeWordTurnStartedAt, now.timeIntervalSince(started) > hardCap {
                    self.finishWakeWordTurn(submit: true)
                    return
                }

                if self.wakeWordTurnHasHeardSpeech {
                    if let lastVoice = self.wakeWordTurnLastVoiceAt,
                       now.timeIntervalSince(lastVoice) > silenceWindow {
                        self.finishWakeWordTurn(submit: true)
                    }
                } else if let started = self.wakeWordTurnStartedAt,
                          now.timeIntervalSince(started) > noSpeechTimeout {
                    // Likely a false trigger — nobody spoke. Drop the turn.
                    self.finishWakeWordTurn(submit: false)
                }
            }
        }
        RunLoop.main.add(timer, forMode: .common)
        wakeWordSilenceTimer = timer
    }

    private func finishWakeWordTurn(submit: Bool) {
        wakeWordSilenceTimer?.invalidate()
        wakeWordSilenceTimer = nil
        ...
        if submit {
            ...
            jarvisDictationManager.cancelCurrentDictation(preserveDraftText: false)
        }

        scheduleWakeWordResume(after: 0.8)
    }

    /// Resumes background wake-word listening once the dictation engine has
    /// fully released the microphone.
    private func scheduleWakeWordResume(after delay: TimeInterval) {
        guard isWakeWordEnabled else { return }
        DispatchQueue.main.asyncAfter(deadline: .now() + delay) { [weak self] in
            guard let self, self.isWakeWordEnabled else { return }
            guard !self.jarvisDictationManager.isDictationInProgress else {
                self.scheduleWakeWordResume(after: 0.5)
                return
            }
            self.wakeWordListener.resume()
        }
    }
    ...
    private func handleShortcutTransition(_ transition: JarvisPushToTalkShortcut.ShortcutTransition) {
        ...
        case .pressed:
            ...
            // Free the mic from the wake-word listener while push-to-talk records.
            wakeWordSilenceTimer?.invalidate()
            wakeWordSilenceTimer = nil
            wakeWordListener.pauseForDictation()

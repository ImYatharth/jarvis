//
//  JarvisScreenCaptureUtility.swift
//  Jarvis
//
//  Standalone screenshot capture for Jarvis's voice-and-screen flow.
//  Decoupled from the old screenshot manager so Jarvis can capture the
//  user's current context without depending on session UI state.
//

import AppKit
import ScreenCaptureKit

struct JarvisScreenCapture {
    let imageData: Data
    let label: String
...
@MainActor
enum JarvisScreenCaptureUtility {

    static func captureCurrentScreenAsJPEG() async throws -> JarvisScreenCapture {
    ...
    static func captureAllScreensAsJPEG() async throws -> [JarvisScreenCapture] {
        let content = try await SCShareableContent.excludingDesktopWindows(false, onScreenWindowsOnly: true)

        guard !content.displays.isEmpty else {
            throw NSError(domain: "JarvisScreenCapture", code: -1,
        ...
        var capturedScreens: [JarvisScreenCapture] = []

        for (displayIndex, display) in sortedDisplays.enumerated() {
            // Use NSScreen.frame (AppKit coordinates, bottom-left origin) so
            // displayFrame is in the same coordinate system as NSEvent.mouseLocation
            // and the overlay window's screenFrame in JarvisCursorOverlayView.
            ...
            capturedScreens.append(JarvisScreenCapture(
                imageData: jpegData,
                label: screenLabel,
        ...
        guard !capturedScreens.isEmpty else {
            throw NSError(domain: "JarvisScreenCapture", code: -2,

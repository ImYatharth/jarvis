...
@MainActor
struct JarvisCursorOverlayView: View {
    ...
    init(
        ...
        // Jarvis cursor doesn't flash at (0,0) before onAppear fires.
    ...
    /// Scale factor applied to the Jarvis cursor during flight. Grows to ~1.3x
    /// at the midpoint of the arc and shrinks back to 1.0x on landing, creating
    /// an energetic "swooping" feel.
    @State private var jarvisFlightScale: CGFloat = 1.0

    /// Scale factor for the navigation speech bubble's pop-in entrance.
    /// Starts at 0.5 and springs to 1.0 when the first character appears.
    @State private var navigationBubbleScale: CGFloat = 1.0

    /// True when the Jarvis cursor is flying BACK to the pointer after pointing.
    /// Only during the return flight can cursor movement cancel the animation.
    @State private var isReturningToCursor: Bool = false

    private let fullWelcomeMessage = "hey, i'm jarvis"
    ...
    var body: some View {
        ZStack {
            ...
            // Navigation pointer bubble — shown when the Jarvis cursor arrives at a detected element.
            // Pops in with a scale-bounce (0.5x → 1.0x spring) and a bright initial
            // glow that settles, creating a "materializing" effect.
            if jarvisNavigationMode == .pointingAtTarget && !navigationBubbleText.isEmpty {
            ...
            JarvisCursorGlyphView(isReturning: isReturningToCursor)
                ...
                .animation(
                    jarvisNavigationMode == .followingCursor
                ...
                .animation(
                    jarvisNavigationMode == .navigatingToTarget ? nil : .easeInOut(duration: 0.3),
                    value: triangleRotationDegrees
                )

            JarvisListeningCursorView(audioPowerLevel: orbitManager.currentAudioPowerLevel)
                .opacity(jarvisCursorIsVisibleOnThisScreen && orbitManager.voiceState == .listening ? cursorOpacity : 0)
                .position(cursorPosition)
                .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
                .animation(.easeIn(duration: 0.15), value: orbitManager.voiceState)

            JarvisProcessingCursorView()
                .opacity(jarvisCursorIsVisibleOnThisScreen && orbitManager.voiceState == .processing ? cursorOpacity : 0)
                .position(cursorPosition)
                .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
                .animation(.easeIn(duration: 0.15), value: orbitManager.voiceState)
        ...
        .onChange(of: orbitManager.detectedElementScreenLocation) { _, newLocation in
            // When a UI element location is detected, navigate the Jarvis cursor to
    ...
    private var jarvisCursorIsVisibleOnThisScreen: Bool {
        switch jarvisNavigationMode {
        case .followingCursor:
            // If another screen's JarvisCursorOverlayView is navigating to an element,
            // hide the cursor on this screen to prevent a duplicate Jarvis cursor
    ...
    private var shouldShowCodexActivityHUD: Bool {
        ...
        // Hide when the response overlay is visible — it already shows status
        // and response text, so the two cards would overlap.
        guard !orbitManager.isResponseOverlayVisible else { return false }
    ...
    private func startTrackingCursor() {
        timer = Timer.scheduledTimer(withTimeInterval: 0.016, repeats: true) { _ in
            Task { @MainActor in
                ...
                // During forward flight or pointing, the Jarvis cursor is not interrupted by
                ...
                if self.jarvisNavigationMode == .navigatingToTarget && self.isReturningToCursor {
                ...
                if self.jarvisNavigationMode != .followingCursor {
                    return
                }

                let swiftUIPosition = self.convertScreenPointToSwiftUICoordinates(mouseLocation)
                let jarvisCursorX = swiftUIPosition.x + 35
                let jarvisCursorY = swiftUIPosition.y + 25
                self.cursorPosition = CGPoint(x: jarvisCursorX, y: jarvisCursorY)
            }
        }
    }
    ...
    /// Starts animating the Jarvis cursor toward a detected UI element location.
    private func startNavigatingToElement(screenLocation: CGPoint) {
        ...
        // Offset the target so the Jarvis cursor sits beside the element rather than
        ...
        jarvisNavigationMode = .navigatingToTarget
        isReturningToCursor = false

        animateBezierFlightArc(to: clampedTarget) {
            guard self.jarvisNavigationMode == .navigatingToTarget else { return }
            self.startPointingAtElement()
        }
    }

    /// Animates the Jarvis cursor along a quadratic bezier arc from its current position
    ...
    private func animateBezierFlightArc(
        ...
        navigationAnimationTask = Task { @MainActor in
            for currentFrame in 1...totalFrames {
                ...
                self.triangleRotationDegrees = atan2(tangentY, tangentX) * (180.0 / .pi) - JarvisBranding.defaultMarkHeadingDegrees

                let scalePulse = sin(linearProgress * .pi)
                self.jarvisFlightScale = 1.0 + scalePulse * 0.3
            }

            guard !Task.isCancelled else { return }
            self.navigationAnimationTask = nil
            self.cursorPosition = endPosition
            self.jarvisFlightScale = 1.0
            onComplete()
        }
    }
    ...
    private func startPointingAtElement() {
        jarvisNavigationMode = .pointingAtTarget
        ...
        streamNavigationBubbleCharacter(phrase: pointerPhrase, characterIndex: 0) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                guard self.jarvisNavigationMode == .pointingAtTarget else { return }
    ...
    private func streamNavigationBubbleCharacter(
        ...
        guard jarvisNavigationMode == .pointingAtTarget else { return }
    ...
    /// Flies the Jarvis cursor back to the current pointer position after pointing is done.
    ...
    private func cancelNavigationAndResumeFollowing() {
        ...
        navigationBubbleScale = 1.0
        jarvisFlightScale = 1.0
        finishNavigationAndResumeFollowing()
    }

    /// Returns the Jarvis cursor to normal pointer-following mode after navigation completes.
...
private enum JarvisCursorMetrics {
...
private struct JarvisOverlayMarkView: View {
    var size: CGFloat = JarvisCursorMetrics.markSize
    var fillColor: Color = Color(hex: "#111111")
    var strokeColor: Color = .white
    var lineWidth: CGFloat = JarvisCursorMetrics.baseLineWidth
...
private struct JarvisDashedMarkStroke: View {
    var size: CGFloat
    var tint: Color
    ...
    var body: some View {
        let perimeter = JarvisBranding.markPerimeter(in: CGSize(width: size, height: size))
...
private struct JarvisMiniSpinner: View {
            ...
            ZStack {
                JarvisMarkShape()
                    .stroke(
                        ...
                        style: StrokeStyle(
                            lineWidth: JarvisCursorMetrics.spinnerLineWidth,
                            lineJoin: .round
                        )
                    )
                    .frame(width: JarvisCursorMetrics.spinnerSize, height: JarvisCursorMetrics.spinnerSize)

                JarvisDashedMarkStroke(
                    size: JarvisCursorMetrics.spinnerSize,
                    tint: tint,
                    lineWidth: JarvisCursorMetrics.spinnerLineWidth,
...
@MainActor
class JarvisOverlayWindowManager {
    ...
    func showOverlay(
        ...
        for screen in screens {
            ...
            let contentView = JarvisCursorOverlayView(

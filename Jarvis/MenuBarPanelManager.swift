...
//  (JarvisPanelView) via NSHostingView. Uses the same NSPanel pattern as
...
extension Notification.Name {
    static let jarvisDismissPanel = Notification.Name("jarvisDismissPanel")
}
...
final class MenuBarPanelManager: NSObject {
    ...
    init(orbitManager: JarvisManager) {
        self.orbitManager = orbitManager
        super.init()
        createStatusItem()

        dismissPanelObserver = NotificationCenter.default.addObserver(
            forName: .jarvisDismissPanel,
            object: nil,
            queue: .main
        ) { [weak self] _ in
    ...
    private func createStatusItem() {
        ...
        button.image = JarvisBranding.makeTemplateMenuBarIcon(size: 18)
    ...
    private func showPanel() {
        ...
        orbitManager.ensureCodexSessionReady()
    ...
    private func createPanel() {
        let jarvisPanelView = JarvisPanelView(orbitManager: orbitManager)
            .frame(width: panelWidth)

        let hostingView = NSHostingView(rootView: jarvisPanelView)
    ...
    private func installClickOutsideMonitor() {
        ...
        clickOutsideMonitor = NSEvent.addGlobalMonitorForEvents(
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                ...
                if !self.orbitManager.allPermissionsGranted && !NSApp.isActive {
                    return
                }

//
//  JarvisDictationManager.swift
//  Jarvis
//
//  Shared push-to-talk dictation manager for Jarvis voice input.
...
enum JarvisDictationPermissionProblem {
    case microphoneAccessDenied
    case speechRecognitionDenied
}

private enum JarvisDictationStartSource {
    case microphoneButton
    case keyboardShortcut
}

private struct JarvisDictationDraftCallbacks {
    let updateDraftText: (String) -> Void
    let submitDraftText: (String) -> Void
}

@MainActor
final class JarvisDictationManager: NSObject, ObservableObject {
    ...
    @Published private(set) var recordedAudioPowerHistory = Array(
        repeating: JarvisDictationManager.recordedAudioPowerHistoryBaselineLevel,
        count: JarvisDictationManager.recordedAudioPowerHistoryLength
    )
    ...
    @Published private(set) var currentPermissionProblem: JarvisDictationPermissionProblem?
    ...
    private var transcriptionProvider: any SpeechToTextProvider
    private let audioEngine = AVAudioEngine()
    private var activeTranscriptionSession: (any SpeechToTextStreamingSession)?
    private var activeStartSource: JarvisDictationStartSource?
    private var draftCallbacks: JarvisDictationDraftCallbacks?
    ...
    override init() {
        let transcriptionProvider = JarvisSpeechToTextProviderFactory.makeDefaultProvider()
        self.transcriptionProvider = transcriptionProvider
        self.transcriptionProviderDisplayName = transcriptionProvider.displayName
        super.init()
        settingsCancellable = JarvisSettings.shared.$voicePreset
            .receive(on: DispatchQueue.main)
            .sink { [weak self] voicePreset in
                self?.updateTranscriptionProvider(for: voicePreset)
            }
    }
    ...
    func refreshConfiguredProviders() {
        ...
        updateTranscriptionProvider(for: JarvisSettings.shared.voicePreset)
    }
    ...
    private func startPushToTalk(
        startSource: JarvisDictationStartSource,
        currentDraftText: String,
        updateDraftText: @escaping (String) -> Void,
        submitDraftText: @escaping (String) -> Void,
        ...
        print("🎙️ JarvisDictationManager: start requested (\(startSource))")

        if needsInitialPermissionPrompt {
            print("🎙️ JarvisDictationManager: requesting initial permissions")
            NSApplication.shared.activate(ignoringOtherApps: true)
        ...
        guard await requestMicrophoneAndSpeechPermissionsWithoutDuplicatePrompts() else {
            print("🎙️ JarvisDictationManager: permissions missing or denied")
            isPreparingToRecord = false
            return
        }
        guard !Task.isCancelled else {
            print("🎙️ JarvisDictationManager: start cancelled (shortcut released during permission check)")
            isPreparingToRecord = false
            return
        }
        guard pendingStartRequestIdentifier == startRequestIdentifier else {
            print("🎙️ JarvisDictationManager: start request superseded")
            isPreparingToRecord = false
            return
        }
        ...
        draftCallbacks = JarvisDictationDraftCallbacks(
            updateDraftText: updateDraftText,
            submitDraftText: submitDraftText
        )
        ...
        guard !Task.isCancelled else {
            print("🎙️ JarvisDictationManager: start cancelled (shortcut released before recording began)")
            resetSessionState()
            return
        }
        ...
        do {
            ...
            guard !Task.isCancelled else {
                print("🎙️ JarvisDictationManager: start cancelled (shortcut released during session start)")
                audioEngine.stop()
                audioEngine.inputNode.removeTap(onBus: 0)
                activeTranscriptionSession?.cancel()
                resetSessionState()
            ...
            isPreparingToRecord = false
            print("🎙️ JarvisDictationManager: recognition session started")
        } catch {
            isPreparingToRecord = false
            ...
            print("❌ JarvisDictationManager: failed to start recognition session (\(transcriptionProvider.displayName)): \(error)")
            resetSessionState()
        }
    }

    private func stopPushToTalk(expectedStartSource: JarvisDictationStartSource) {
        pendingStartRequestIdentifier = UUID()
        ...
        print("🎙️ JarvisDictationManager: stop requested (\(expectedStartSource))")
    ...
    private func startRecognitionSession() async throws {
        ...
        print("🎙️ JarvisDictationManager: opening transcription provider \(transcriptionProvider.displayName)")
        ...
        print("🎙️ JarvisDictationManager: provider ready, starting audio engine")
    ...
    private func handleRecognitionError(_ error: Error) {
        ...
        if isFinalizingTranscript && !latestRecognizedText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            print("❌ Jarvis dictation error (\(transcriptionProvider.displayName)): \(error)")
            lastErrorMessage = userFacingErrorMessage(
                from: error,
    ...
    private func updateTranscriptionProvider(for voicePreset: JarvisVoicePreset) {
        guard !isDictationInProgress else { return }
        let updatedProvider = JarvisSpeechToTextProviderFactory.makeProvider(for: voicePreset)
        transcriptionProvider = updatedProvider
        transcriptionProviderDisplayName = updatedProvider.displayName
    }
